...
public class CommunicationService {
    ...
    @Transactional
    public ChatMessage sendMessage(HttpServletRequest request, String doctorId, String patientId, String senderRole, String senderId, String message, List<String> fileUrls) {
    ...
    @Transactional
    public void deleteMessage(HttpServletRequest request, String messageId) {
        String token = JwtUtil.extractToken(request);
        Role role = getRole(token);
        String jwtUserId = getUserId(token);
    ...
    @Transactional
    public List<ChatMessage> getChatHistory(HttpServletRequest request, String doctorId, String patientId) {
        String token = JwtUtil.extractToken(request);
    ...
    @Transactional
    public void markChatAsRead(HttpServletRequest request, String doctorId, String patientId) {
        String token = JwtUtil.extractToken(request);
        Role role = getRole(token);
        String uid = getUserId(token);
        ...
        if (role == Role.PATIENT) {
            ...
            if (!patient.getUserId().equals(uid))
                throw new RuntimeException("Unauthorized patient");
            List<ChatMessage> unread = chatRepo.findByDoctorIdAndPatientIdAndReadFalseAndSenderRole(
    ...
    @Transactional
    public List<PatientWithUserDTO> getDoctorChatPatients(HttpServletRequest request, String doctorId) {
        String token = JwtUtil.extractToken(request);
        Doctor doctor = doctorRepo.findById(doctorId)
                .orElseThrow(() -> new RuntimeException("Doctor not found"));
    ...
    @Transactional
    public List<PatientWithUserDTO> getDoctorNewPatients(HttpServletRequest request, String doctorId) {
        String token = JwtUtil.extractToken(request);
        Doctor doctor = doctorRepo.findById(doctorId)
                .orElseThrow(() -> new RuntimeException("Doctor not found"));
        if (!doctor.getUserId().equals(getUserId(token)))
    ...
    @Transactional
    public List<DoctorWithUserDTO> getPatientChatDoctors(HttpServletRequest request, String patientId) {
        String token = JwtUtil.extractToken(request);
        Patient patient = patientRepo.findById(patientId)
                .orElseThrow(() -> new RuntimeException("Patient not found"));
    ...
    @Transactional
    public List<DoctorWithUserDTO> getPatientNewDoctors(HttpServletRequest request, String patientId) {
        String token = JwtUtil.extractToken(request);
        Patient patient = patientRepo.findById(patientId)
                .orElseThrow(() -> new RuntimeException("Patient not found"));
        if (!patient.getUserId().equals(getUserId(token)))
